...
//(all the even values from nums) and the second item in the Array being the
//odds array(all the odd values from nums).
...
*/
  ...
  //Code Here
function removeItem(arr1, remArr1) {
  if (arguments.length <= 0) {
    return [];
  }
  if (arr1.indexOf(remArr1) !== -1) {
    arr1.splice(arr1.indexOf(remArr1), 1);
  }
  return arr1;
}

function addItem(arr1, addArr1) {
  if (arguments.length <= 0) {
    return [];
  }
  if (arr1.indexOf(addArr1) === -1) {
    arr1.splice(arr1.indexOf(addArr1), 0, addArr1);
  }
  return arr1;
}
